...
T = int(input('Enter an integer value for the terminal time of the contract, T: ')) # terminal time ;; for e.g: if s_end = 2*k, let T = 1,
N = int(input('Enter an integer value for the number of spatial nodes, N: ')) # spatial values ;; let N = 100,
M = int(input('Enter an integer value for the number of time steps, M: ')) # time values/hops (M ~ N^2) ;;  and let M = 10,000.
...
if ( dt <= ((h**2) / ( (sigma**2) * (4 * (k**2)) )) ):    # Stability condition: dt <= (h**2) / ( (sigma**2) * (4 * (k**2)) )
   ...
   t[0] = t_init 
    ...
    M = int(input('Stability condition not satisfied. The chosen value for M returns a numerical approximation that does not converge. Enter a larger value for the number of time-steps, M: '))
...
for m in range(0, M):
    for j in range(0, N+1):
        if j == 0:
            u[j,m+1] = 0 # Boundary condition; setting equal to k yields a better approximation for the PUT-Option
        elif j == N:
            u[j,m+1] = (s_end - k) # Boundary Condition; force the value of the approximation to be the intial condition at s_end <==> j == N+1
        else:
            u[j,m+1] = u[j,m] + (( ((alpha[j])**2) * dt)/(2 * h**2)) * ( u[j+1,m] - 
            2 * u[j,m] + u[j-1,m] ) + ((beta[j]) * ((dt)/(2*h))) * ( u[j+1,m] - u[j-1,m])
            - (r * dt) * u[j,m]
...
plt.title('Black-Scholes Approx. with Exact Solution at Boundary')
