...
##
## In particular, we will work with the C.E.V. Model.
import numpy as np
...
alpha = np.zeros(N+1)
gamma = np.zeros(N+1)
beta = float(2.0/3) # CEV coefficient; NOTE: beta == 1 corresponds to Black-Scholes model; when beta =/= 1, use something to the effect of beta = float(1.0/2), or float(2.0/3)
for j in range(1, N):
    alpha[j] = ( (sigma) * ((s[j])**beta) )    
    gamma[j] = r * s[j]
    
# definition of the solution u(t,s) to our PDE
...
plt.title('CEV approx. with beta = 1 and Exact Solution at Boundary')
